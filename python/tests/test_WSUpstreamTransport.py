...
class WSUpstreamTransportTests(unittest.TestCase):
    def test_reconnect_closes_old_socket_and_ignores_stale_reader_errors(self) -> None:
        ...
        transport = WSUpstreamTransport({"upstream_ws_cdp_url": "ws://127.0.0.1:1/devtools/browser/test"})
        ...
        with patch("modcdp.transport.WSUpstreamTransport.create_connection", side_effect=[first, second]):
            transport.connect()
