...
from modcdp.transport.ReverseWSUpstreamTransport import ReverseWSUpstreamTransport
...
class ReverseWSUpstreamTransportTests(unittest.TestCase):
    ...
    def test_close_rejects_pending_peer_waits(self) -> None:
        ...
        transport = ReverseWSUpstreamTransport({"upstream_reversews_bind": f"127.0.0.1:{reverse_port}", "upstream_reversews_wait_timeout_ms": 5_000})
    ...
    def test_close_resets_peer_wait_state(self) -> None:
        reverse_port = _free_port()
        transport = ReverseWSUpstreamTransport({"upstream_reversews_bind": f"127.0.0.1:{reverse_port}", "upstream_reversews_wait_timeout_ms": 5})
        transport.connect()
        url = transport.url
        if url is None:
    ...
    def test_accepts_replacement_peer_after_disconnect(self) -> None:
        reverse_port = _free_port()
        transport = ReverseWSUpstreamTransport({"upstream_reversews_bind": f"127.0.0.1:{reverse_port}", "upstream_reversews_wait_timeout_ms": 500})
        transport.connect()
        url = transport.url
        if url is None:
            self.fail("reverse transport url was not set")
