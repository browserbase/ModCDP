...
export class NATSUpstreamTransport extends UpstreamTransport {
  ...
  constructor(options: UpstreamOptions = {}) {
    ...
    this.upstream_nats_wait_timeout_ms =
  ...
  >(
    ...
    if (
      ...
      this.publish(this.outgoingSubject(), {
        ...
        ...(this.upstream_nats_role === "client"
          ? { reply_subject: this.client_reply_subject }
          : {}),
  ...
  update(config: UpstreamOptions = {}) {
    if (config.upstream_nats_url || config.upstream_nats_subject_prefix) {
      const normalized = normalizeNatsUrl(
        config.upstream_nats_url ??
          this.upstream_nats_url ??
  ...
  async connect() {
    ...
    if (!this.upstream_nats_url)
      throw new Error(
        "upstream.upstream_mode=nats requires upstream_nats_url.",
      );
  ...
  async waitForPeer() {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const timeout = setTimeout(() => {
        ...
        reject(
          new Error(
            `Timed out waiting ${wait_timeout_ms}ms for NATS ModCDP peer.`,
          ),
        );
      }, wait_timeout_ms);
  ...
  private async connectTcp(url: URL) {
    ...
    const node_transport =
      url.protocol === "tls:"
        ? await import(node_tls_module_path)
        : await import(node_net_module_path);
  ...
  private subscribe() {
    ...
    if (this.upstream_nats_role === "client") {
      this.writeProtocol(
        `SUB ${this.client_reply_subject} ${this.next_sid++}\r\n`,
      );
    }
  }
