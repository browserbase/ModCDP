...
export class BBExtensionInjector extends ExtensionInjector {
  ...
  private async uploadExtension(zip_path: string) {
    ...
    if (!browserbase_api_key) {
      throw new Error(
        "BBExtensionInjector requires BROWSERBASE_API_KEY or injector.injector_bb_api_key.",
      );
    }
    const base_url =
      firstString(this.injector_bb_base_url, process.env.BROWSERBASE_BASE_URL) ??
