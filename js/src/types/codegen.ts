...
for (const base of modcdpTypes
  ...
  const lines =
    base === "AddCustomCommand"
      ? [
          `      ${methodName}: async (params_or_name?: cdp.types.ts.Mod.${base}Params | string, options: ModCustomCommandOptions = {}) => {`,
      ...
      : base === "AddCustomEvent"
        ? [
            `      ${methodName}: async (params_or_name?: cdp.types.ts.Mod.${base}Params | string, options: { event_schema?: z.ZodType | null } = {}) => {`,
            `        const input = typeof params_or_name === "string" ? { name: params_or_name, event_schema: options.event_schema ?? null } : params_or_name;`,
            `        const parsed = Mod.${base}Params.parse(input ?? {});`,
          ]
        : [
            `      ${methodName}: async (params?: cdp.types.ts.Mod.${base}Params) => {`,
