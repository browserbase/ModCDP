...
import { createCdpAliases, type CdpAliases } from "./generated/aliases.js";
import {
...
export type CDPEventSpec<TEventSchema extends z.ZodType = z.ZodType> = {
  event_schema?: TEventSchema | null;
};

export type CDPCommandMap = Record<string, CDPCommandSpec>;
export type CDPEventMap = Record<string, CDPEventSpec>;
export type CDPTypesCustomCommands<
  TCommands extends CDPCommandMap = CDPCommandMap,
> =
  | ModCDPAddCustomCommandParams[]
  | {
      [TName in keyof TCommands]: TCommands[TName];
    };
export type CDPTypesCustomEvents<TEvents extends CDPEventMap = CDPEventMap> =
  | ModCDPAddCustomEventObjectParams[]
  | {
      [TName in keyof TEvents]: TEvents[TName];
    };
export type CDPTypesOptions<
...
export type CDPTypesCommandRegistration = ModCDPAddCustomCommandParams & {
  params_schema?: z.ZodType | null;
  result_schema?: z.ZodType | null;
};
export type CDPTypesEventRegistration = ModCDPAddCustomEventObjectParams & {
  event_schema?: z.ZodType | null;
};
export type CDPAliasSend = (
  method: string,
  params?: unknown,
) => Promise<unknown>;
export type CDPEventNameInput =
  | string
  | symbol
  | (z.ZodType & ModCDPNamedValue);
export type CDPEventPayload<TEvent extends z.ZodType> =
...
type CommandNamesForDomain<
  TCommands extends CDPCommandMap,
  TDomain extends string,
> = Extract<keyof TCommands, `${TDomain}.${string}`>;
type UnwrapSingleKey<TValue> = TValue extends object
  ? keyof TValue extends infer TKey
    ? TKey extends keyof TValue
      ? keyof TValue extends TKey
        ? TValue[TKey]
        : TValue
      : TValue
    : TValue
  : TValue;
type CommandAliasParams<TSpec extends CDPCommandSpec> =
  TSpec["params_schema"] extends z.ZodType
    ? z.input<TSpec["params_schema"]>
    : ProtocolParams;
type CommandAliasResult<TSpec extends CDPCommandSpec> =
  TSpec["result_schema"] extends z.ZodType
    ? UnwrapSingleKey<z.output<TSpec["result_schema"]>>
    : ProtocolResult;
export type CDPCommandAliases<TCommands extends CDPCommandMap> = {
  [TDomain in DomainName<Extract<keyof TCommands, string>>]: {
    [TName in CommandNamesForDomain<TCommands, TDomain> as MethodName<
      Extract<TName, string>
    >]: undefined extends CommandAliasParams<TCommands[TName]>
      ? (
          params?: CommandAliasParams<TCommands[TName]>,
        ) => Promise<CommandAliasResult<TCommands[TName]>>
      : (
          params: CommandAliasParams<TCommands[TName]>,
        ) => Promise<CommandAliasResult<TCommands[TName]>>;
  };
};
export type CDPEventMapPayloads<TEvents extends CDPEventMap> = {
  [TName in keyof TEvents]: TEvents[TName]["event_schema"] extends z.ZodType
    ? z.output<TEvents[TName]["event_schema"]>
    : ProtocolPayload;
};

export function hasCommandExpression(
...
function serializablePayloadSchema(
  schema: ModCDPPayloadSchemaSpec | null | undefined,
) {
  if (!schema) return null;
  const normalized_schema = normalizeModCDPPayloadSchema(schema);
  return normalized_schema
    ? (z.toJSONSchema(normalized_schema) as ModCDPPayloadSchemaSpec)
    : null;
}
...
> {
  ...
  readonly custom_commands: ModCDPAddCustomCommandParams[];
  readonly custom_events: ModCDPAddCustomEventObjectParams[];
  ...
  constructor(options: CDPTypesOptions<TCommands, TEvents> = {}) {
    ...
    for (const middleware of options.custom_middlewares ?? [])
      this.addCustomMiddleware(middleware);
  }

  update<
    TMoreCommands extends CDPCommandMap = Record<never, never>,
    TMoreEvents extends CDPEventMap = Record<never, never>,
  >(
    options: CDPTypesOptions<TMoreCommands, TMoreEvents>,
  ): CDPTypes<TCommands & TMoreCommands, TEvents & TMoreEvents> {
    const updated = new CDPTypes<
      TCommands & TMoreCommands,
      TEvents & TMoreEvents
    >({
      custom_commands: [
        ...this.custom_commands,
        ...this.customCommandEntries(options.custom_commands ?? []),
      ],
      custom_events: [
        ...this.custom_events,
        ...this.customEventEntries(options.custom_events ?? []),
      ],
      custom_middlewares: [
        ...this.custom_middlewares,
        ...(options.custom_middlewares ?? []),
      ],
    });
    for (const binding of this.alias_bindings)
      updated.installAliases(binding.target, binding.send);
    return updated;
  }

  nativeCommandSchema(method: string) {
    return (
      (nativeCommandSchemas as Record<string, CdpCommandSchema>)[method] ?? null
    );
  }
  ...
  normalizeEventName(event_name: CDPEventNameInput) {
    ...
    return typeof event_name === "symbol"
      ? event_name
    ...
    return {
      ...
      custom_command_name:
        method === "Mod.addCustomCommand"
          ? normalizeModCDPName(
              Mod.AddCustomCommandParams.parse(command_params).name,
            )
          : null,
    };
  }
  ...
  parseCommandParams(method: string, params: unknown = {}) {
    return (this.command_params_schemas.get(method)?.parse(params ?? {}) ??
      params ??
      {}) as ProtocolParams;
  }
  ...
  }: { expression_required?: boolean } = {}) {
    return this.custom_commands
      .filter(
        (command) => !expression_required || hasCommandExpression(command),
      )
      .map((command) => ({
  ...
  customEventWireRegistrations() {
    return this.custom_events.map((event) => ({
  ...
  installAliases(target: object, send: CDPAliasSend) {
    if (!this.alias_bindings.some((binding) => binding.target === target))
      this.alias_bindings.push({ target, send });
    ...
    for (const command of this.custom_commands)
      this.installCustomCommandAlias(
        target,
        normalizeModCDPName(command.name),
        send,
      );
  ...
  private customEventWireRegistration(name: string) {
    return (
      this.customEventWireRegistrations().find(
  ...
  private upsertCustomCommand(command: ModCDPAddCustomCommandParams) {
    const existing_index = this.custom_commands.findIndex(
      (entry) => normalizeModCDPName(entry.name) === command.name,
    );
    if (existing_index >= 0) this.custom_commands[existing_index] = command;
    else this.custom_commands.push(command);
  }

  private upsertCustomEvent(event: ModCDPAddCustomEventObjectParams) {
    const existing_index = this.custom_events.findIndex(
      (entry) => normalizeModCDPName(entry.name) === event.name,
    );
    if (existing_index >= 0) this.custom_events[existing_index] = event;
    else this.custom_events.push(event);
  }
